package com.github.huoyu820125.idstar.service;
 ...
 */
@Service
public class IdStarService implements InitializingBean {
    ...
    public Long idle(Integer version) {
        ...
        Long serialNo = 0L;
        long regionNo = 0;
        synchronized (IdStarService.class) {
...
//            log.info("jar相对路径(IDE调试下不可用，真实运行时是jar相对路径)={}", jar_path);
        ...
        String jarFullPath = IdStarService.class.getProtectionDomain().getCodeSource().getLocation().getPath();
...
//        log.info("jar绝对路径(IDE调试下是不带target/class位置，实际运行时是jar绝对路径+file:前缀)={}", jarFullPath);
    ...
    */
    public List<NodeDto> allNode() {
        ...
        IdStarServiceClient client = new IdStarServiceClient(cluster.masterAddress());
        try {
